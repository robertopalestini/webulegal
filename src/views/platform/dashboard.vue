...
<template>
  ...
  <main>
            ...
            <div class="col-md-6 text-center">
              ...
              <div class="d-flex justify-content-center">
                <ul
                  ...
                  <li>
                    <a style="border-radius:30px;font-weight:600;padding:4px;background: rgb(236 234 234 / 46%);font-size:14px;padding-left: 10px;padding-right: 10px;cursor:pointer"
                      @click="changeMode('document'); searchInput.holder = 'Buscar doctrina, jurisprudencia y legislación'"
                      v-bind:class="{ activeMode: (searchMode === 'document') }">Bibilioteca</a>
                  </li>
                  <li>
                    <a style="border-radius:30px;font-weight:600;padding:4px;background: rgb(236 234 234 / 46%);font-size:14px;padding-left: 10px;padding-right: 10px;cursor:pointer"
                      @click="changeMode('writing'); searchInput.holder = 'Buscar modelos de escritos automatizados'"
              ...
              <form style="position:relative" @submit.prevent="gotoSearch()">
                <div style="position:relative">
                  <input type="text" class="form-control imput-search input-search-dashboard" @keyup="search"
                    v-model="searchInput.target" :placeholder="searchInput.holder">
    ...
    margin-left: 24px;" />
                ...
                <div
                  style="position:absolute;top:0;width:100%;height:auto;background:white;z-index:500;border:1px solid #CCCCCC;border-radius:8px;margin-top:60px;max-height:400px;overflow:hidden;overflow-y:auto"
                  v-if="searchInput.target">
                  <ul style="width:100%;padding:0;margin:0;list-style:none;">

                    <li
                      style="padding:15px;width:100%;text-align:left;background:rgb(248 248 248);border-bottom:1px solid #e6e6e6"
                      v-for="(searchTagItem, index)   in searchTags" v-if="searchTags.length > 0">
                      <a href="#" style="font-size:11px;font-weight:600;"
                        @click.prevent="gotoTag(searchTagItem.data.title, searchTagItem._id)">{{
                            searchTagItem.data.title
                        }}</a>
                    </li>


                    <li
                      style="background:rgb(241 241 241);padding:7px;padding-top:7px;padding-bottom:7px;text-align:left;font-weight:600;color:#636363;font-size:12px;"
                      v-if="searchDocuments.length > 0"> TEXTUAL</li>
                    <li
                      style="padding:15px;width:100%;text-align:left;background:rgb(248 248 248);border-bottom:1px solid #e6e6e6"
                      v-for="(searchDocument, index)   in searchDocuments">

                      <a href="#" style="font-size:13px;font-weight:600;color:black"
                        @click="goToDocument(searchDocument._id, searchDocument.data.type, $event)">{{
                            searchDocument.data.title
                        }}</a>
            ...
            <div class="col-md-8"
              ...
              <div style="overflow-y: scroll;overflow-x:hidden;max-height: 40vh;">
                <div class="row" style="padding-top:15px;padding-bottom:15px;border-bottom:1px solid #D1D1D1;"
                  v-for="document in BestDocuments" v-if="BestDocuments.length > 0 || bestWritings.empty">
            ...
            <div class="col-md-8" v-if="searchMode === 'document'" style="margin-top:15px;">
              <div class="row" v-if="BestDocuments.length == 0 || BestDocuments.empty == true">
                <div class="col-md-12 text-left"
                  style="border-radius:8px;color:white;font-weight:500;background:#5D5D5D;padding:15px">
  ...
  methods: {
    ...
    search() {
      ...
      this.searchAllDocuments().then((data) => {
        ...
        console.log('sear', this.searchDocuments)
      })
