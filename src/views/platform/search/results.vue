...
export default {
            ...
            for (var i = item.data.tags.length - 1; i >= 0; i--) {
              ...
              for (var e = this.tagFilters.length - 1; e >= 0; e--) {
                var rowTarget = this.tagFilters[e];
                  console.log('loop tags', item.data.title , row._id , rowTarget.id)
  ...
  methods: {
    getFolderAll() {
      ...
      fetch(window.ENDPOINT + "/folders/v2/default/documents", requestOptions)
    ...
    getChilds(parent) {
      ...
      fetch(
        window.ENDPOINT + "/folders/v2/default/documents/childs",
        requestOptions
      )
        .then((response) => response.json())
        .then((data) => {
    ...
    selectTag(id, title) {
      ...
      console.log('tags selected ' , this.tagFilters);
      this.getResults();
    },
    ...
    getResults() {
      return new Promise((resolve, reject) => {
        ...
        fetch(
          window.ENDPOINT + "/comunity/search/complete/documents",
          requestOptions
        )
          .then((response) => response.json())
          .then((data) => {
            console.log('send request results ' , data);
            if (data.length == 0) {
