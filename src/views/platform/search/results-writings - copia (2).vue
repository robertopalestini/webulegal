    ...
    <main>
        <div class="container-fluid">
            <div class="row">
                ...
                <div class="col-md-10" style="height:calc(100vh - 70px);overflow:hidden">   
                        <!-- ======================================= -->

                            <div class="row">
                                <div class="col-md-3" style="height:calc(100vh - 70px);overflow:hidden;overflow-y:auto;padding-top:15px">
                                    <div class="col-12" style="padding:0;">
                                    <div class="row">
                                    <div class="col-2 text-center" style="padding-top:5px;padding-bottom:5px;" >
    <a href="@" @click.prevent="$router.go(-1)" > <img src="@/assets/flecha-izquierda.svg" style="height:15px;width:15px;" /> </a>
</div>
...
"  v-model="searchInput.target" />
             <button type="submit" hidden="true" ></button>
                                    ...
                                    <div class="col-md-12" style="padding:0px;font-size:13px;margin-bottom:15px">
                                    ...
                                    <div class="col-md-12" style="padding:0;">
                                       <b  v-if="foldersFilters.length == 0 && tagFilters.length == 0 " style="font-size:12px;">No se han colocado filtros.</b>
                                        <ul style="margin:0;padding:0;list-style:none">
                                            <li style="padding:7px" v-for="(item,index) in foldersFilters" v-if="foldersFilters.length > 0"><a href="#" style="padding: 5px 8px; background: rgb(236, 236, 236); border-radius: 30px; font-size: 9.5px; font-weight: 600; margin-right: 10px; margin-bottom: 5px;">{{item.title}} <a href="#" style="margin-left:10px" @click="removeFilterFolder(index)"><i class="fas fa-times"></i></a> </a></li>
                                            <li v-if="foldersFilters.length > 0">
                                               <hr />
                                            </li>
                                            <li style="padding:7px" v-for="(item,index) in tagFilters" v-if="tagFilters.length > 0"><a href="#" style="padding: 5px 8px; background: rgb(236, 236, 236); border-radius: 30px; font-size: 9.5px; font-weight: 600; margin-right: 10px; margin-bottom: 5px;">{{item.title}}  <a href="#" style="margin-left:10px" @click="removeFilterTag(index)"><i class="fas fa-times"></i></a></a></li>
                                        </ul>
                                    </div>

                                    <div class="col-md-12" style="padding: 15px;background: #e3e1e1;padding-top: 3px;padding-bottom: 3px;font-size: 13px;font-weight: 600;color: #505050;margin-bottom: 15px;margin-top: 15px;text-transform: uppercase;">
                                        Carpetas
                                    </div>
                                    <div class="col-md-12">
...
">
                                    ...
                                    <b  v-if="searchTags.length == 0" style="font-size:12px;">No se han encontrado etiquetas.</b>
                                        <ul style="margin:0;padding:0;list-style:none">
                                            <li style="padding:7px" v-for="(item,index) in searchTags" v-if="searchTags.length > 0"><a href="#" style="padding: 5px 8px; background: rgb(236, 236, 236); border-radius: 30px; font-size: 9.5px; font-weight: 600; margin-right: 10px; margin-bottom: 5px;" @click.prevent="selectTag(item.data.title)">{{item.data.title}}  </a></li> 
                                        </ul>
                                    </div>


                                </div>
                                <div class="col-md-9" style="height:calc(100vh - 70px);overflow:hidden;overflow-y:auto;padding-top:15px">
                                  <div class="row" style="border:1px solid #d0cece;border-radius:8px;width:100%;">
                                     <div class="col-md-12" style="
                                     padding: 15px;background: #e3e1e1;padding-top: 3px;padding-bottom: 3px;font-size: 13px;font-weight: 600;color: #505050;margin-bottom: 15px; text-transform: uppercase;border-top-left-radius: 8px;border-top-right-radius: 8px;">Resultados de busqueda</div>
      <div style="height:80vh;overflow:hidden;overflow-y:auto;width:100%">
       



                                  <div class="col-md-12" style="border-bottom:1px solid #d0cece;width:100%" v-for="(item,index) in searchDocuments" v-if="searchDocuments.length > 0"  >
       <div class="row">
       <div class="col-md-9">
       ...
       <div class="col-md-3"  v-html="displayStars(item.data.cout_validate)">  
        </div>
       </div>
                                  </div>
...
import Tree from 'vuejs-tree'
 ...
 export default {
  ...
  methods: { 
    getFolderAll() {
            ...
            fetch(window.ENDPOINT + '/folders/v2/default/writings', requestOptions).then(response => response.json()).then((data) => {
 ...
 getChilds(parent) {
            ...
            fetch(window.ENDPOINT + '/folders/v2/default/writings/childs', requestOptions).then(response => response.json()).then((data) => {
 ...
 pathMoveSelectedRemove(index,nodeid) {
   ...
   getResults() { 
      ...
      const requestOptions = {
         ...
         fetch( window.ENDPOINT + '/search/results/writings', requestOptions).then(response => response.json()).then((data) => { 
          if(data.empty) {
    ...
    searchAllDocuments() {
      ...
      const requestOptions = {
         ...
         fetch(this.endpointSearchWriting, requestOptions).then(response => response.json()).then((data) => { 
          if(data.empty) {
            resolve([]); 
          } else {
            resolve(data); 
    ...
    searchAllFoldersDocuments() { 
      ...
      const requestOptions = {
         ...
         fetch(window.ENDPOINT + '/search/folders/writings', requestOptions).then(response => response.json()).then((data) => { 
          if(data.empty) {
            resolve([]); 
          } else {
            resolve(data); 
