<template>
         ...
         <div class="row">
             <div class="col-md-2" style="height:calc(100vh - 70px);overflow:hidden;overflow-y:auto;padding:0;"> 
                ...
                <div class="col-md-10 text-center" style="padding:20px;border:1px solid #F3F3F3;background:rgb(255,255,255,0.1);height:calc(100vh - 70px);overflow:hidden;overflow-y:auto;position:relative;"> 
                  ...
                  <h4 style="font-weight:bold;text-align:left;font-size:17px">Reportes</h4>
             </div>
 ...
 <div class="col-md-12 text-left" style="padding:15px;font-size:12px;margin-bottom:15px;border:1px solid #E7E7E7" v-for="item in data"  v-if="data.length > 0">
                 ...
                 <p style="margin-top:5px;margin-bottom:0"><b>Documento</b> : {{item.data.document.title}}</p> 
                 <p style="margin-top:5px;margin-bottom:0"><b>Descripcion</b> : {{item.data.document.description}}</p>
                 <p style="margin-top:5px;margin-bottom:0"><b>Id</b> :  {{item.data.document.id}}</p>

                 <hr /> 
                  <a href="#" @click.prevent="removeAction(item.data.document.id,item._id)" style="font-weight:500;margin-left:15px" ><img src="@/assets/trash.svg" style="width:14px;height:14px;margin-right:5px" /> Eliminar Documento y reporte</a>
...
export default {
  data() {
    return {
      ...
      endpoint: window.ENDPOINT + '/requests/get/all/report',
      inputCreate:null,
      tags:[],
      history:[],
      searchTarget:null,
