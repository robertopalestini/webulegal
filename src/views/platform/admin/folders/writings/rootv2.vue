...
export default {
    data() {
        return {
            ...
            endpointGetAll: window.ENDPOINT + '/folders/v2/default/writings',

            endpointSave: window.ENDPOINT + '/folders/v2/default/writings/save/root',
           ...
           nameNewFolder : { title : null },
            ...
            nodeSelect :  [{text:'rara',id:23,nodes:[]}],
            pathMoveSelected:[],
            treeDisplayData: [],
            temp:{
                index : null,
      ...
      if (parentNodeId === undefined) { 
       ...
       var test =    this.$refs["my-tree"].nodes.findIndex((x) => x.id == this.temp.node.id)  
        ...
        if(parentNode.nodes === undefined) {
            ...
            // this.saveFolders();
            this.saveDeleteFolder(this.temp.node.id)
         return;
      ...
      openModalEdit(folder,index)  {
        ...
        saveEditFolder() {
            ...
            const requestOptions = {
                ...
                body: JSON.stringify({
                    auth: this.auth,
                    text : this.nameEditFolder.title,
                    id : this.nodeSelect.nodes[this.targetEditIndex].id,
                })
            };
            fetch(window.ENDPOINT + '/folders/v2/default/writings/edit', requestOptions).then(response => response.json()).then((data) => {
        ...
        saveRootFolder() {
            ...
            fetch(window.ENDPOINT + '/folders/v2/default/writings/save/root', requestOptions).then(response => response.json()).then((data) => {
        ...
        saveChildFolder() {
            ...
            const requestOptions = {
                ...
                body: JSON.stringify({
                    auth: this.auth,
                    text : this.nameNewFolder.title,
                    parent : this.nodeSelect.id
                })
            };
            fetch(window.ENDPOINT + '/folders/v2/default/writings/save/child', requestOptions).then(response => response.json()).then((data) => {
        ...
        saveDeleteFolder(id) {
            ...
            fetch(window.ENDPOINT + '/folders/v2/default/writings/delete', requestOptions).then(response => response.json()).then((data) => {
                this.$Progress.finish();
            })
        },
