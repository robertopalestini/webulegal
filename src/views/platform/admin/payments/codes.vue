...
<template>
          ...
          <div class="row justify-content-center">
            ...
            <div class="col-md-12">
              <div class="col-md-12" style="margin-bottom:30px">
                <h4 style="font-weight:bold;text-align:left;font-size:17px">Codigos promocionales <button type="submit"
                    class="btn btn-primary" style="width:auto;position:relative;z-index:1000"
                    @click.prevent="openNew()">Añadir nuevo</button></h4>
              </div>
              ...
              <table class="table">
                ...
                <tbody>
                  <tr v-for="(item,index) in data" v-if="data.length > 0">
                    ...
                    <td><span @click.prevent="openRegisters(item.registers)"
                        style="cursor:pointer"><b>{{item.registers.length}}</b> registrados</span></td>
                    <td>
                      <a href="#" @click.prevent="openEdit(index)"><img src="@/assets/edit.svg"
                          style="width:14px;height:14px;margin-right:5px" /> Editar</a>

                      <a href="#" @click.prevent="removeAction(item._id)" style="font-weight:500;margin-left:15px"><img
                          src="@/assets/trash.svg" style="width:14px;height:14px;margin-right:5px" /> Eliminar</a>
  ...
  <!-- Modal -->
  <div class="modal fade" id="createPayment" data-backdrop="static" data-keyboard="true" tabindex="-1"
  ...
  <!-- Modal -->
  <div class="modal fade" id="editPayment" data-backdrop="static" data-keyboard="true" tabindex="-1"
    aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
  ...
  <!-- Modal -->
  <div class="modal fade" id="openRegisters" data-backdrop="static" data-keyboard="true" tabindex="-1"
    aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <form class="modal-content">
        ...
        <div class="modal-body" v-if="registers.length > 0">
          ...
          <div class="col-md-12 text-left"
            style="padding:15px;font-size:12px;margin-bottom:15px;border:1px solid #E7E7E7" v-for="item in registers">
            <img :src="'https://ui-avatars.com/api/?background=000000&color=fff&name=' + item.firstname + item.lastname"
              style="height:30px;border-radius:100%" /> <span style="font-weight:600;font-size:14px;">
  ...
  methods: {
    ...
    load() {
     ...
     for (let index = 0; index < data.length; index++) {
       if(data[index].registers == null) {
        data[index].registers = [];
       }
       if(!data[index].registers) {
        data[index].registers = [];
       }  
     }
