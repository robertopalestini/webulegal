...
// const store = useLibraryStore() //don't forget to register the store

const selectTag = (tag, idtag) => {
  tagsSelectedTags.push({
    title: tag,
    id: idtag,
  });

  console.log(tagsSelectedTags)
}


// const loadAllDocuments = () => {
//   store.loadAllDocuments() //you can do like this or by simply @click="store.loadAllDocuments()"
// }
</script>
...
<template>
  <div class="col-12" style="padding-top: 10px; padding-bottom: 10px">
    ...
    <div class="col-12 text-left" style="padding: 0" v-if="tagsSelectedTags.length > 0">
      ...
      <ul
        ...
        <li
          ...
          v-for="(tagSelectedTag, index) in tagsSelectedTags">

          <a href="#"
            style="font-size: 12px;padding-top: 5px;padding-bottom: 5px;background: rgb(206, 206, 206);border-radius: 30px;display: flex;
                            place-content: flex-start;align-items: center;flex-direction: row;font-weight: 600;padding-left: 10px;padding-right: 10px;">

            {{ tagSelectedTag.title }}
            <a href="#" style="margin-left: 10px" @click="removeFilterTags(index, tagSelectedTags.title)">
    ...
    <ul style="padding: 0; margin: 0; width: 100%; list-style: none">
      ...
      <li style="padding: 0;margin: 0;width: 100%;list-style: none;line-height: 15px;margin-bottom: 15px;"
        v-for="item in items" :key="index" @click="selectTag(item.id, item.data.title)">
